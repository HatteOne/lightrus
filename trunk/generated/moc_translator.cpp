/****************************************************************************
** Meta object code from reading C++ file 'translator.h'
**
** Created: Mon May 25 21:50:31 2009
**      by: The Qt Meta Object Compiler version 61 (Qt 4.5.1)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../inc/translator.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'translator.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 61
#error "This file was generated using the moc from 4.5.1. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_Translator[] = {

 // content:
       2,       // revision
       0,       // classname
       0,    0, // classinfo
       4,   12, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors

 // signals: signature, parameters, type, tag, flags
      12,   11,   11,   11, 0x05,
      31,   11,   11,   11, 0x05,
      51,   11,   11,   11, 0x05,
      82,   11,   11,   11, 0x05,

       0        // eod
};

static const char qt_meta_stringdata_Translator[] = {
    "Translator\0\0StartTranslating()\0"
    "FinishTranslating()\0StartNewFileTranslate(QString)\0"
    "CurrentFileProgressChanged(int)\0"
};

const QMetaObject Translator::staticMetaObject = {
    { &LogObject::staticMetaObject, qt_meta_stringdata_Translator,
      qt_meta_data_Translator, 0 }
};

const QMetaObject *Translator::metaObject() const
{
    return &staticMetaObject;
}

void *Translator::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_Translator))
        return static_cast<void*>(const_cast< Translator*>(this));
    return LogObject::qt_metacast(_clname);
}

int Translator::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = LogObject::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        switch (_id) {
        case 0: StartTranslating(); break;
        case 1: FinishTranslating(); break;
        case 2: StartNewFileTranslate((*reinterpret_cast< const QString(*)>(_a[1]))); break;
        case 3: CurrentFileProgressChanged((*reinterpret_cast< int(*)>(_a[1]))); break;
        default: ;
        }
        _id -= 4;
    }
    return _id;
}

// SIGNAL 0
void Translator::StartTranslating()
{
    QMetaObject::activate(this, &staticMetaObject, 0, 0);
}

// SIGNAL 1
void Translator::FinishTranslating()
{
    QMetaObject::activate(this, &staticMetaObject, 1, 0);
}

// SIGNAL 2
void Translator::StartNewFileTranslate(const QString & _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 2, _a);
}

// SIGNAL 3
void Translator::CurrentFileProgressChanged(int _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 3, _a);
}
QT_END_MOC_NAMESPACE
